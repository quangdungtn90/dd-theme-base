...
/**
 * DD_Base_Breadcrumb class.
 *
 * @class        DD_Base_Breadcrumb
 * @package        DD_Base
 * @since 1.0
 * @category    Class
 * @author        ThemesPond
...
class DD_Base_Breadcrumb {
	...
	public function get_breadcrumb() {
		return apply_filters( 'dd_base\get_breadcrumb', $this->crumbs, $this );
	}
	...
	public function generate() {
		...
		if ( ( ! is_front_page() && ! is_post_type_archive() ) || is_paged() ) {
			...
			$title = esc_html__( 'Posts', 'dd-base' );
	...
	private function add_crumbs_home() {
		...
		if ( $blog = get_option( 'page_for_posts' ) ) {
			$title = get_the_title( $blog, esc_url( home_url() ) );
			$link  = esc_url( get_permalink( $blog ) );
		} else {
			$title = esc_attr__( 'Posts', 'dd-base' );
	...
	private function add_crumbs_404() {
		$this->add_crumb( esc_html__( 'Error 404', 'dd-base' ) );
	}
	...
	private function add_crumbs_single( $post_id = 0, $permalink = '' ) {
		...
		/**
		 * Comment this code below to avoid duplicate title
		 *
	...
	private function add_crumbs_tag() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Posts tagged &ldquo;%s&rdquo;', 'dd-base' ), single_tag_title( '', false ) ), get_tag_link( $queried_object->term_id ) );
	}
	...
	private function add_crumbs_author() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Author: %s', 'dd-base' ), $userdata->display_name ) );
	}
	...
	private function search_trail() {
		if ( is_search() ) {
			$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'dd-base' ), get_search_query() ), remove_query_arg( 'paged' ) );
		}
	}
	...
	private function paged_trail() {
		if ( get_query_var( 'paged' ) ) {
			$this->add_crumb( sprintf( esc_html__( 'Page %d', 'dd-base' ), get_query_var( 'paged' ) ) );
		}
	}
